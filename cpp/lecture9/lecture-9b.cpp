...
// create a stuct to input multiple parameters
struct thread_data {
   int thread_id;
   int delay;
   std::string message;
...
void *PrintHello( void *inputdata ) {
   ...
   my_data = (struct thread_data *) inputdata;

   time_t create_time = time(NULL);
   //To access elements using pointer notation you must use -> obj-> instead of dot notation obj.
